...
class MLApplication:
    ...
    def create_control_elements(self):
        ...
        self.use_model_button = ttk.Button(self.control_frame, text="Use Model",
                                           command=self.use_model, state="disabled")
        self.use_model_button.pack(side="left", padx=5)
    ...
    def start_training(self):
        ...
        def train_thread():
            try:
                ...
                self.use_model_button.config(state="normal")
            except Exception as e:
    ...
    def use_model(self):
        def upload_and_predict():
            try:
                transcript_path = filedialog.askopenfilename(filetypes=[("Text and DOCX files", "*.txt;*.docx")])
                xlsx_path = filedialog.askopenfilename(filetypes=[("Excel files", "*.xlsx")])

                if not transcript_path or not xlsx_path:
                    self.output_queue.put("Please upload both a transcript and an Excel file.")
                    return
                ...
                self.output_queue.put("Transcript uploaded successfully.")

                # Load Excel file and predict retention percentages
                xlsx_data = pd.read_excel(xlsx_path)
                self.output_queue.put("Excel file loaded successfully.")
                ...
                dummy_features = {
                    'Shares': 100,
                    'Comments added': 50,
                    'Likes (vs. dislikes) (%)': 95.0,
                    'Average view duration': 120,
                    'Subscribers': 200,
                    'Impressions click-through rate (%)': 10.0
                }
                ...
                for pos in positions:
                    features = np.array([[
                        dummy_features['Shares'] + pos,
                        dummy_features['Comments added'] + pos,
                        dummy_features['Likes (vs. dislikes) (%)'],
                        dummy_features['Average view duration'],
                        dummy_features['Subscribers'],
                        dummy_features['Impressions click-through rate (%)']
                    ]])
                    predictions[pos] = self.model.predict(features)

                # Populate predictions into Excel and display
                for pos, value in predictions.items():
                    xlsx_data.at[pos, 'Retention Percentage'] = value[0]

                self.output_queue.put("Predictions generated and populated into Excel.")

                # Save updated Excel
                updated_path = xlsx_path.replace('.xlsx', '_updated.xlsx')
                xlsx_data.to_excel(updated_path, index=False)
                self.output_queue.put(f"Updated Excel file saved at: {updated_path}")

            except Exception as e:
                self.output_queue.put(f"Error during prediction: {str(e)}")

        threading.Thread(target=upload_and_predict, daemon=True).start()
